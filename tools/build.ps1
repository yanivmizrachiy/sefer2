...
foreach($g in @($cfg.entities.grades)){
  ...
  foreach($grp in @($cfg.entities.groups) | Where-Object { [string]$_.grade -eq $gid }){
    ...
    $htmlG += "<section class='card'><div style='display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:center'><div><div style='font-weight:800'>$nm</div><div class='sub'>מורה: "+( ([string]::IsNullOrWhiteSpace($t)){"טרם הוגדר"}else{$t})+"</div></div><a class='btn' href='../groups/$id.html'>כניסה</a></div></section>"
  }
